...
/// Current local time formatted as Haskell's `formatTime defaultTimeLocale "%a %d-%m-%Y %R"`,
/// e.g. `Thu 02-07-2026 14:30` — the body of the `auto_comments=[{date}]` comment line. Because it
/// reads the wall clock it is inherently non-deterministic, so it is *not* exercised by the
/// byte-parity suite (the format string is the parity contract).
const WDAY_ABBR: [&str; 7] = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];

#[cfg(unix)]
fn script_run_date() -> String {
    let now = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|d| d.as_secs() as libc::time_t)
        .unwrap_or(0);
    let mut tm: libc::tm = unsafe { std::mem::zeroed() };
    // `localtime_r` applies the local timezone, matching Haskell's `getZonedTime`.
...
#[cfg(not(unix))]
fn script_run_date() -> String {
    // No libc on non-Unix targets: fall back to UTC (no local-timezone shift). NGLess targets Unix,
    // so this branch is a best-effort placeholder. Civil-from-days is Howard Hinnant's algorithm.
    ...
    let wday = (days.rem_euclid(7) + 4) % 7; // 1970-01-01 was a Thursday (wday 4)
    let z = days + 719_468;
    ...
    format!(
        "{} {:02}-{:02}-{:04} {:02}:{:02}",
        WDAY_ABBR[wday as usize], d, m, y, hour, min
    )
}
...
fn build_comment(
    ...
    for a in auto {
        match a.as_str() {
            // `Script run on <local time>` — mirrors `buildAutoComment AutoDate`.
            "date" => out.push(format!("Script run on {}", script_run_date())),
...
mod tests {
    ...
    fn build_comment_unknown_auto_errors() {
        assert!(build_comment(None, &["bogus".to_string()], "", None).is_err());
    }

    #[test]
    fn build_comment_date_format() {
        // `auto_comments=[{date}]` yields one `Script run on <weekday DD-MM-YYYY HH:MM>` line.
        // The clock value is non-deterministic, so assert the shape, not the instant.
        let cs = build_comment(None, &["date".to_string()], "", None).unwrap();
        assert_eq!(cs.len(), 1);
        let line = &cs[0];
        assert!(line.starts_with("Script run on "), "got {line:?}");
        let body = line.trim_start_matches("Script run on ");
        // "Thu 02-07-2026 14:30"
        let parts: Vec<&str> = body.split(' ').collect();
        assert_eq!(parts.len(), 3, "got {line:?}");
        assert_eq!(parts[0].len(), 3, "weekday abbrev: {line:?}");
        assert_eq!(parts[1].len(), 10, "DD-MM-YYYY: {line:?}");
        assert_eq!(parts[2].len(), 5, "HH:MM: {line:?}");
        assert_eq!(parts[1].matches('-').count(), 2, "got {line:?}");
        assert_eq!(parts[2].matches(':').count(), 1, "got {line:?}");
    }
